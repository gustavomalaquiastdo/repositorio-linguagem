...
#2º Ano Matutino
#Aluno: Gustavo Malaquias Teixeira de Oliveira
...
#Exceçaõ em cpf: apenas números, campo vazio.
            while True:
                try:
                    ...
                    print(
                        "CPF foi preenchido incorretamente. Use apenas números."
                    )
                else:
                    break
...
#Exceçaõ em endereço: campo vazio.
            ...
            senhafunc = '8234'  #senha de acesso do sistema
            while True:
...
class Item_Pedido:
    ...
    def cardapio(self):
        while True:
            ...
            if menu == '1':
                print('-------------------')
                print(" '1'- ", sucoUva.nome,
                      "   300ml: R$4,00  500ml: R$6,00")
                print(" '2'- ", sucoLimao.nome,
                      "   300ml: R$3,00  500ml: R$5,00")
                print(" '3'- ", sucoLaranja.nome,
                      "   300ml: R$3,00  500ml: R$5,00")
                print(" '4'- ", sucoMaracuja.nome,
                      "   300ml: R$3,00  500ml: R$5,00")
                print(" '5'- ", sucoMorango.nome,
                      "   300ml: R$4,00  500ml: R$6,00")
                print(" '6'- ", sucoAcerola.nome,
                      "   300ml: R$3,00  500ml: R$5,00")
                print('-------------------')

                pedido = input("Digite pedido: ")
                if pedido == '1':
                    sucoLimao.escolherQntd(1)
                    valorT(sucoLimao.preco)  #valorT=Valor Total
                    #inserindo suco de uva na lista de itens pedidos
                    self.inserirProdutos(sucoLimao)
                elif pedido == '2':
                    sucoUva.escolherQntd(0)
                    valorT(sucoUva.preco)
                ...
                pagar = input("\nDigite ok para finalizar compra| ")
                if pagar == 'ok':
                    print('\n----------------------')
                    print(
                        'Agradecemos por comprar no DIXES LANCHES! \nAguarde que seu pedido logo chegará no endereço,',
                        cliente.endereco,
                        ', caso o endereço de entrega for outro, por favor, informe a seguir:'
                    )
...
Digite 2 para fazer pedido| """)
                if menuCliente == '1':
                    ...
                    itempedido.cardapio(
                    )  #vamos utilizar objetos para manipular
                    break
